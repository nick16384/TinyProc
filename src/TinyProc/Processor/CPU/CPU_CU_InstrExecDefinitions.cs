...
public partial class CPU
{
    private partial class ControlUnit
    {
        ...
        // TODO: These functions would probably be implemented differently on hardware. Find out how exactly.
        /// <summary>
        ...
        /// <summary>
        /// Load data from an absolute memory address in the source register into the destination register.
        /// </summary>
        private void INSTRUCTION_R_LDR_A()
        {
            Logging.LogDebug(
                ...
                $"Src:{RegisterNameAndValueFormatted(_currentInstruction.R_SrcRegCode)}");
            CopyFromRegisterToRegister(_currentInstruction.R_SrcRegCode, InternalRegisterCode.RCODE_SPECIAL_MAR);
            // FIXME: Enable memory read
            CopyFromRegisterToRegister(InternalRegisterCode.RCODE_SPECIAL_MDR, _currentInstruction.R_DestRegCode);
            // FIXME: Disable memory read
            Logging.LogDebug($"New value: {RegisterNameAndValueFormatted(_currentInstruction.R_DestRegCode)}");
        }
        /// <summary>
        /// Load data from a relative memory offset (PC-relative) in the source register into the destination register.
        /// </summary>
        ...
        /// <summary>
        /// Store data to an absolute memory address contained in the source register from the destination register.
        /// </summary>
        private void INSTRUCTION_R_STR_A()
        {
            ...
            CopyFromRegisterToRegister(_currentInstruction.R_DestRegCode, InternalRegisterCode.RCODE_SPECIAL_MDR);
            // FIXME: Disable memory write
        }
        /// <summary>
        /// Store data to a relative memory offset (PC-relative) in the source register from the destination register.
        /// </summary>
        ...
        /// <summary>
        /// Push the value in the source register onto the stack.
        /// </summary>
        private void INSTRUCTION_R_PUSH()
        {
            Logging.LogDebug($"Push register Src:{RegisterNameAndValueFormatted(_currentInstruction.R_SrcRegCode)} to stack\n" +
                $"SP: {_mmu.SP.ValueDirect:x8}");
            PushOntoStack(_currentInstruction.R_SrcRegCode);
        }
        /// <summary>
        /// Pop the uppermost stack value into the destination register.
        /// </summary>
        private void INSTRUCTION_R_POP()
        {
            Logging.LogDebug($"Pop from stack to register Dst:{RegisterNameAndValueFormatted(_currentInstruction.R_DestRegCode)}\n"
                + $"SP: {_mmu.SP.ValueDirect:x8}");
        ...
        /// <summary>
        /// Perform an arithmetic operation between a register and an immediate value and store the result in the register.
        /// </summary>
        ...
        /// <summary>
        /// Load data from an absolute immediate memory address into the destination register.
        /// </summary>
        private void INSTRUCTION_I_LD_A()
        {
            Logging.LogDebug(
                ...
                $"#{_currentInstruction.I_ImmediateValue:x8}");
            CopyFromRegisterToRegister(InternalRegisterCode.RCODE_SPECIAL_IRB, InternalRegisterCode.RCODE_SPECIAL_MAR);
            // FIXME: Enable memory read
            CopyFromRegisterToRegister(InternalRegisterCode.RCODE_SPECIAL_MDR, _currentInstruction.I_DestRegCode);
            // FIXME: Disable memory read
            Logging.LogDebug($"New value: {RegisterNameAndValueFormatted(_currentInstruction.I_DestRegCode)}");
        }
        /// <summary>
        /// Load data from a relative immediate memory offset (PC-relative) into the destination register.
        /// </summary>
        ...
        /// <summary>
        /// Store data to an absolute immediate memory address from the register.
        /// </summary>
        private void INSTRUCTION_I_ST_A()
        {
            ...
            CopyFromRegisterToRegister(InternalRegisterCode.RCODE_SPECIAL_IRB, InternalRegisterCode.RCODE_SPECIAL_MAR);
            // FIXME: Enable memory write
            CopyFromRegisterToRegister(_currentInstruction.I_DestRegCode, InternalRegisterCode.RCODE_SPECIAL_MDR);
            // FIXME: Disable memory write
        }
        /// <summary>
        /// Store data to a relative immediate memory offset (PC-relative) from the destination register.
        /// </summary>
        ...
        private void INSTRUCTION_J_NOP()
        {
            ...
            // Do literally nothing. Same as jumping to next instruction, which is automatically done.
        }
        /// <summary>
        /// Jump to an absolute immediate address.
        /// </summary>
        ...
        /// <summary>
        /// Jump to a relative immediate offset (PC-relative).
        /// </summary>
        ...
        /// <summary>
        /// Call a subroutine at an absolute immediate address.
        /// Does the same as JMP_A except it also pushes the PC to the stack.
        /// </summary>
        ...
        /// <summary>
        /// Call a subroutine at a relative immediate offset (PC-relative).
        /// Does the same as JMP_R except it also pushes the PC to the stack.
        /// </summary>
        ...
        /// Pops the uppermost stack element into PC.
        /// </summary>
        ...
        /// Saves the entire CPU state, calculates the interrupt address from vector and calls the address.
        /// </summary>
        ...
        /// Restores the entire CPU state and returns to the address where the interrupt was caused.
        /// </summary>
